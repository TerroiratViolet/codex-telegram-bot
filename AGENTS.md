...
5. Never log tarot questions, user answers, generated psychological hypotheses, or full Telegram updates.
6. Tarot analysis is an administrator-only reflective aid, not diagnosis or treatment. Keep consent,
   uncertainty language, crisis escalation guidance, and the admin-only delivery boundary intact.
7. For every behavior change, add or update a test in `tests/`.
8. Run all acceptance commands before finishing:
...
9. Summarize changed behavior, files, test results, and any deployment impact.
10. Do not deploy directly from a feature branch. Open a pull request and merge only after CI passes.
