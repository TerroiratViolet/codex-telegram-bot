...
群组中显示用户授权图片与按钮
        ↓ 仅被邀请用户可以点击
问题 A → 随机 78 张标准塔罗牌面 → 回答 B → 回答 C
        ↓
内存会话状态机（两小时过期，不写日志或数据库）
        ↓
牌面结构化资料 + A/B/C → 已配置的占卜助手 provider（默认 Gemini）
        ↓
管理员私聊中的参考分析
        ↓
管理员人工判断并手动回复用户
...
这个系统对外称为“占卜”，用户核心体验是塔罗占卜；但系统目标不是预测未来。牌面被
当作投射性反思的镜子，用来承接用户自己的意象、联想和叙事。占卜助手的分析应帮助
管理员理解参与者的情绪、价值观、内在冲突、关系模式、成长课题与潜意识关注点。

用户描述优先于标准牌义。若用户在问题 B/C 中看到的内容、情绪或故事与传统牌义不同，
分析必须以用户自己的描述为主，标准牌义只作为辅助材料。所有输出都应服务于理解、
探索与叙事重构，而不是宿命论预测。
...
`tarot_cards.py` 定义 78 张 Rider–Waite–Smith 标准牌组。每张牌包含：
...
Gemini 偶尔会在正式分析请求中返回 `503 Service Unavailable`，随后 `/llmcheck`
又立刻恢复正常。这类情况说明 API Key、模型名称和权限是可用的，底层原因通常是模型
服务的瞬时不可用，而不是 Bot 配置错误。

为避免一次短暂抖动导致管理员收不到分析，`GeminiTarotAnalyzer` 会对 `500`、`502`、
`503`、`504`、超时和临时不可用文本进行短间隔重试。OpenAI provider 对连接错误、
超时和同类 `5xx` 状态码也使用相同保护。重试仍然不会记录用户 A/B/C 原文；如果重试
耗尽，Bot 才会向管理员发送安全失败原因。
